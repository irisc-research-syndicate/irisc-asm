        ...
        .map(|value| match value {
            ...
            number_or_range => {
                if let Some((low, high)) = number_or_range.split_once("..") {
                    ...
                    Ok((0..high.wrapping_sub(low))
                        .map(|x| x.wrapping_add(low))
                        .collect())
                } else {
                    let number = parse_number(number_or_range)
                        .context(format!("Invalid number: {}", number_or_range))?;
                    Ok(vec![number])
                }
            }
        })
...
pub fn parse_parameter(s: &str) -> Result<(String, Vec<u64>)> {
    ...
    Ok((
        key.to_string(),
        parse_ranges(ranges).context(format!("Invalid ranges: {}", ranges))?,
    ))
}
...
mod test {
    ...
    fn test_parse_number() {
        ...
        assert_eq!(
            parse_number("0xffffffffffffffff").unwrap(),
            0xffffffffffffffff
        );
        ...
        assert_eq!(
            parse_number("-0x7fffffffffffffff").unwrap(),
            0x8000000000000001
        );
        assert!(parse_number("-0x8000000000000000").is_err());

        assert_eq!(
            parse_number("-1").unwrap(),
            parse_number("0xffffffffffffffff").unwrap()
        );
    }
    ...
    fn test_parse_ranges() {
        ...
        assert_eq!(
            parse_ranges("-10..10").unwrap(),
            (-10..10).map(|x| x as u64).collect::<Vec<_>>()
        );
        assert_eq!(
            parse_ranges("0xfffffffffffffff0..0xffffffffffffffff").unwrap(),
            (-16..-1).map(|x| x as u64).collect::<Vec<_>>()
        );
    }
    ...
    fn test_parse_ranges_random() {
        ...
        assert_eq!(
            parse_ranges("rand8:16")
                .unwrap()
                .into_iter()
                .map(|x| x & 0xffffffffffffff00)
                .collect::<Vec<_>>(),
            vec![0u64; 16]
        );
        ...
        assert_eq!(
            parse_ranges("rand16:16")
                .unwrap()
                .into_iter()
                .map(|x| x & 0xffffffffffff0000)
                .collect::<Vec<_>>(),
            vec![0u64; 16]
        );
        ...
        assert_eq!(
            parse_ranges("rand32:16")
                .unwrap()
                .into_iter()
                .map(|x| x & 0xffffffff00000000)
                .collect::<Vec<_>>(),
            vec![0u64; 16]
        );
    ...
    fn test_parse_parameter() {
        assert_eq!(
            parse_parameter("r5=1,2,3,10..20,-10..-5,0xfedcba9876543210").unwrap(),
            (
                "r5".to_string(),
                vec![
                    1,
